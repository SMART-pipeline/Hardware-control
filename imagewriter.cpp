...
ImageWriter::ImageWriter(const QString &filePath, size_t imageNumber):m_dataStrip(512),m_compression(5){
...
bool ImageWriter::addImage(const cv::Mat &image){
    ...
    for(int i=0;i<image.rows;i+=h,pData+=bufferSize){
        int h1=std::min(h,image.rows-i);unsigned long dstLen;

        if(8==m_compression){
//            unsigned long dstLen=bufferSize,srcLen=w*h1*2;
//            compress2(dst,&dstLen,pData,srcLen,4);fwrite(dst,dstLen,1,m_file);
        }else if(5==m_compression){
            size_t dstLen2=bufferSize,srcLen=w*h1*2;LZWCodeC::compress(pData,srcLen,dst,dstLen2);
            uchar *pDst=dst;pDst=pData;dstLen2=srcLen;dstLen=dstLen2;
